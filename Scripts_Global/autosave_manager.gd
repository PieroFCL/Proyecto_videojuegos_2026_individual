extends Node
# Gestiona guardado y carga en múltiples slots.

# Ruta del slot automático (checkpoint).
const AUTO_SAVE_PATH = "user://save_auto.tres"
# Rutas de los tres slots manuales.
const MANUAL_SAVE_PATHS = {
	1: "user://save_manual_1.tres",
	2: "user://save_manual_2.tres",
	3: "user://save_manual_3.tres"
}
# Ruta legacy para compatibilidad (se mantiene).
const SAVE_PATH = AUTO_SAVE_PATH

# Escena de transición con fundido.
const FADE_SCENE = preload("res://Shared_Assets/Effects/FadeTransition/fade_transition.tscn")
# Indica si se está cargando una partida.
static var is_loading = false

# Identificador del slot automático.
const AUTO_SLOT_ID = 0
# Identificadores de los slots manuales.
const MANUAL_SLOT_IDS = [1, 2, 3]

# Slot pendiente de cargar al iniciar el mundo.
static var pending_load_slot: int = -1

# Inicializa, elimina archivo antiguo si existe.
func _ready() -> void:
...
# Devuelve la ruta del archivo según el slot.
...
func load_game(slot_id: int) -> bool:
	...
	# Asegura que el jugador sea visible tras cargar.
	var player = get_tree().get_first_node_in_group("player")
	if player:
		player.visible = true
...
# Lista los slots manuales que tienen archivo de guardado.
...
# Elimina todos los archivos de guardado (todos los slots).
func reset_all_saves() -> void:
	print("=== reset_all_saves llamado (pila: ", get_stack(), ")")
...
# Guarda el estado en el slot automático (legacy).
func save_checkpoint() -> void:
	save_game(AUTO_SLOT_ID)

# Carga el último checkpoint desde el slot automático (legacy).
