...
func add_item(item_id: String, amount: int = 1) -> void:
	...
	print("🔧 Inventario: añadido ", amount, " x ", item_id, " → total: ", _items[item_id])
...
func remove_item(item_id: String, amount: int = 1) -> bool:
	...
	print("🔧 Inventario: eliminado ", amount, " x ", item_id, " → nuevo total: ", _items.get(item_id, 0))
	return true
...
func get_item_resource(item_id: String) -> ItemResource:
	...
	print("🔧 get_item_resource: buscando '", path, "', existe? ", exists)
	if exists:
		var res = load(path)
		print("🔧 recurso cargado: ", res)
		return res
	return null
